...
| 2026-08-24 | **Deployment track abierto (post-v1.0).** Artefactos listos: backend/Dockerfile + .dockerignore (non-root), render.yaml blueprint (free tier), guía Supabase+Render en backend/README, toggle ALLOW_REGISTRATION con test → suite 155/155. Pendiente de acciones del usuario: crear proyecto Supabase y servicio Render, luego regenerar APK con URL definitiva. |
